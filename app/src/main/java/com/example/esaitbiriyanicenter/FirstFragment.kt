...
class FirstFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recycler_view.adapter = context?.let { AdapterSectionRecycler(it,sections,this.findNavController()) };
        ...
        proceed_button.setOnClickListener(View.OnClickListener {
            ...
            var mbQuantity = Integer.parseInt(recycler_view.findViewHolderForAdapterPosition(1)?.itemView?.findViewById<TextView>(R.id.quantity)?.text.toString());
            var cbQuantity = Integer.parseInt(recycler_view.findViewHolderForAdapterPosition(2)?.itemView?.findViewById<TextView>(R.id.quantity)?.text.toString());
            var khuskaQuantity = Integer.parseInt(recycler_view.findViewHolderForAdapterPosition(3)?.itemView?.findViewById<TextView>(R.id.quantity)?.text.toString());
            var chicken65Quantity = Integer.parseInt(recycler_view.findViewHolderForAdapterPosition(4)?.itemView?.findViewById<TextView>(R.id.quantity)?.text.toString());
            var vegBiriyaniQuantity = Integer.parseInt(recycler_view.findViewHolderForAdapterPosition(6)?.itemView?.findViewById<TextView>(R.id.quantity)?.text.toString());

            var orderSummary = "";
            var grandTotal= 0;
            if(mbQuantity>0) {
                orderSummary = "Mutton Biriyani x " + mbQuantity + "                                          ₹" + (mbQuantity* 270) + "\n";
            ...
            if(grandTotal>0) {
                ...
                var grandTotalStr = "Grand Total "+"                                               ₹" + (grandTotal+30);
                orderSummary+= "Delivery Charges                                             ₹30";
                val bundle = bundleOf("orderSummary" to orderSummary, "grandTotal" to grandTotalStr);
                this.findNavController()
                    ?.navigate(R.id.action_FirstFragment_to_SecondFragment, bundle);
            } else {
                Toast.makeText(context,"Please select some items from the menu",Toast.LENGTH_SHORT).show();
            }
        })
